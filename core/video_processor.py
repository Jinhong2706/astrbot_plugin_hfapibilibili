...
from astrbot.api.message_components import Video, File, Image, Plain
...
async def merge_audio_video(video_path: Path, audio_path: Path, output_path: Path,
...
async def download_and_process_video(event, bvid: str, bili_api, session, proxy, quality,
    ...
    if not streams:
        yield event.plain_result("未找到可用的视频流，请稍后重试。")
        return
    ...
    if audio_url and temp_audio_path:
        video_task = asyncio.create_task(
            download_file(video_url, temp_video_path, session=session, proxy=proxy,
                          aria2_path=aria2_path, download_method=download_method)
        )
        audio_task = asyncio.create_task(
            download_file(audio_url, temp_audio_path, session=session, proxy=proxy,
                          aria2_path=aria2_path, download_method=download_method)
        )
        video_success, audio_success = await asyncio.gather(video_task, audio_task)
    else:
        video_success = await download_file(
            video_url, temp_video_path, session=session, proxy=proxy,
            aria2_path=aria2_path, download_method=download_method
        )
        audio_success = True
