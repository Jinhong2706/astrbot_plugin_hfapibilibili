...
class BiliAPI:
    ...
    async def get_video_info_by_aid(self, aid: int) -> dict:
        return await self._fetch(f"/bilibili/video/avid/{aid}")
    ...
    async def get_download_urls_by_aid(self, aid: int) -> dict:
        return await self._fetch(f"/bilibili/video/download/avid/{aid}")

    async def search(self, keyword: str, page: int = 1, page_size: int = 20) -> dict:
        encoded = quote(keyword)
        return await self._fetch(f"/bilibili/search/{encoded}", params={"page": page, "page_size": page_size})