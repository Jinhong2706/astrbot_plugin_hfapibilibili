...
class BiliAPI:
    ...
    async def _do_request(self, url: str, params: dict = None) -> dict:
    ...
    async def _fetch(self, endpoint: str, params: dict = None) -> dict:
        ...
        try:
            return await with_retry(
                self._do_request, url, params,
                max_retries=5, delay=5, label=f"API {endpoint}"
            )
    ...
    async def get_download_urls(self, bvid: str, quality: str = "720p") -> dict:
        endpoint = (
            f"/bilibili/video/download/1080/{bvid}" if quality == "1080p"
            else f"/bilibili/video/download/{bvid}"
        )
        return await self._fetch(endpoint)

    async def get_download_urls_by_aid(self, aid: int, quality: str = "720p") -> dict:
        endpoint = (
            f"/bilibili/video/download/aid/1080/{aid}" if quality == "1080p"
            else f"/bilibili/video/download/aid/{aid}"
        )
        return await self._fetch(endpoint)
